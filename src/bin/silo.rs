            ...
            match client.post(&token_url).form(&params).send().await {
                 Ok(resp) => {
                     if resp.status().is_success() {
                         ...
                         match client.post(&exchange_url)
                             ...
                             .await 
                         {
                             Ok(ex_resp) => {
                                 if ex_resp.status().is_success() {
                                     ...
                                     let cert_data: serde_json::Value = serde_json::from_str(&ex_body).unwrap();
                                     ...
                                     let identity = cert_data["identity"].as_str().unwrap_or("unknown");

                                     // 6. Context Injection
                                     let home = dirs::home_dir().unwrap();
                                     let current_dir = home.join(".silo/certs/current");
                                     std::fs::create_dir_all(&current_dir).expect("Failed to create current context dir");

                                     std::fs::write(current_dir.join("gateway.crt"), cert_pem).expect("Failed to write cert");
                                     std::fs::write(current_dir.join("gateway.key"), key_pem).expect("Failed to write key");
                                     std::fs::write(current_dir.join("ca.crt"), ca_pem).expect("Failed to write CA");
                                     ...
                                     println!("❌ Certificate Exchange Failed: {}", ex_resp.status());
                                     std::process::exit(1);
                                 }
                             },
                             Err(e) => {
