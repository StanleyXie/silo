use crate::config::{Config, StorageConfig, VaultConfig, GatewayConfig, GatewayTlsConfig, ControlPlaneConfig, ControlPlaneTlsConfig, AuthConfig};
...
impl BootstrapManager {
        ...
        let config = Config {
            ...
            control_plane: ControlPlaneConfig {
                ...
                tls: ControlPlaneTlsConfig {
                    ca_cert: certs_dir.join("internal/ca.crt").to_string_lossy().to_string(),
                    server_cert: certs_dir.join("internal/server.crt").to_string_lossy().to_string(),
                    server_key: certs_dir.join("internal/server.key").to_string_lossy().to_string(),
                    client_cert: certs_dir.join("internal/client.crt").to_string_lossy().to_string(),
                    client_key: certs_dir.join("internal/client.key").to_string_lossy().to_string(),
                },
            },
    ...
    pub async fn start_vault_dev(&self) -> Result<std::process::Child, Box<dyn std::error::Error>> {
        ...
        for _ in 0..10 {
            if client.get("http://127.0.0.1:8200/v1/sys/health").send().await.is_ok() {
                return Ok(child);
            }
    ...
    pub fn start_silo_server(&self, detach: bool) -> Result<std::process::Child, Box<dyn std::error::Error>> {
        let mut server_path = "silo-server".to_string();
        
        // Try local build if not in path
        if Command::new("silo-server").arg("--version").status().is_err() {
