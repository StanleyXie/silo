...
use proto::v1::{GetRequest, GetResponse, PutRequest, PutResponse, DeleteRequest, DeleteResponse};
use proto::v1::control_service_client::ControlServiceClient;
use tonic::{Request, Response, Status};
use tonic::transport::{Server as TonicServer, Identity, ServerTlsConfig, Certificate, ClientTlsConfig};

use backend::{VaultClient, StorageBackend};
#[cfg(feature = "etcd")]
...
impl ControlService for ControlPlane {
    async fn authorize(&self, request: Request<AuthorizeRequest>) -> Result<Response<AuthorizeResponse>, Status> {
        ...
        info!("[gRPC] Authorize request: {} from {}", req.path, req.identity);
        
        let authorized = self.allowed_identities.contains(&req.identity);
        let reason = if authorized { "Allowed".to_string() } else { format!("Identity '{}' not authorized", req.identity) };
...
impl StorageService for ControlPlane {
    ...
    async fn put(&self, request: Request<PutRequest>) -> Result<Response<PutResponse>, Status> {
        let req = request.into_inner();
        match self.storage.put(&req.path, &req.data, req.base_version).await {
            Ok(version) => Ok(Response::new(PutResponse { success: true, version })),
            Err(e) => Err(Status::internal(e.to_string())),
        }
    }
    ...
    async fn list(&self, request: Request<proto::v1::ListRequest>) -> Result<Response<proto::v1::ListResponse>, Status> {
        let req = request.into_inner();
...
impl ProxyHttp for Gateway {
    ...
    async fn request_filter(&self, session: &mut Session, ctx: &mut Self::CTX) -> Result<bool> {
        ...
        let identity = bearer_identity
            .or(mtls_identity)
            .or_else(|| {
                session.get_header("X-Forwarded-User")
        ...
        let ua = session.get_header("User-Agent")
            .and_then(|h| h.to_str().ok())
            .map(|s| s.to_string())
            .unwrap_or_else(|| "-".to_string());

        match self.handler.handle(session, &ctx.request_id, &identity, &client_ip).await {
            Ok(handled) => {
                let latency = ctx.start_time.elapsed();
                let status = session.response_written().map(|h| h.status.as_u16()).unwrap_or(0);
                if handled {
                     info!("[{}] {} {} -> status:{} latency:{:?} identity:{} ua:\"{}\"", 
                        ctx.request_id,
            ...
            Err(e) => {
                let latency = ctx.start_time.elapsed();
                error!("[{}] Request Filter Error: {} (latency: {:?})", ctx.request_id, e, latency);
                match session.respond_error_with_body(500,  bytes::Bytes::from(format!("Internal Error: {}", e))).await {
                    Ok(_) => {},
    ...
    async fn upstream_peer(&self, _session: &mut Session, _ctx: &mut Self::CTX) -> Result<Box<HttpPeer>> {
        ...
        Ok(Box::new(HttpPeer::new("127.0.0.1:1", false, "".to_string())))
    }
}
...
fn main() {
    ...
    let storage: Arc<dyn StorageBackend> = match silo_cfg.storage.storage_type.as_str() {
        ...
        "etcd" => {
            ...
            let backend = rt.block_on(async {
                EtcdBackend::new(&endpoints).await
    ...
    let cp_addr: std::net::SocketAddr = silo_cfg.control_plane.address.parse().expect("Invalid control plane address");
    ...
    rt.spawn(async move {
        let cert = tokio::fs::read(&cp_tls_cfg.server_cert).await.expect("server cert missing");
        let key = tokio::fs::read(&cp_tls_cfg.server_key).await.expect("server key missing");
        let ca_cert = tokio::fs::read(&cp_tls_cfg.ca_cert).await.expect("ca cert missing");
        ...
        let cp_c = ControlPlane { storage: cp_storage.clone(), allowed_identities: cp_allowed.clone() };
        let cp_s = ControlPlane { storage: cp_storage, allowed_identities: cp_allowed };

        info!("Internal gRPC Control Plane starting on {} (mTLS ENFORCED)", cp_addr);

        TonicServer::builder()
            .tls_config(tls).expect("failed to config gRPC TLS")
    ...
    let control_client = rt.block_on(async {
        let cert = tokio::fs::read(&cp_tls_cfg.client_cert).await.expect("client cert missing");
        let key = tokio::fs::read(&cp_tls_cfg.client_key).await.expect("client key missing");
        let ca_cert = tokio::fs::read(&cp_tls_cfg.ca_cert).await.expect("ca cert missing");
        ...
        let channel = tonic::transport::Endpoint::from_shared(format!("https://{}", silo_cfg.control_plane.address))
            .expect("Invalid channel URL")
            .tls_config(tls).expect("failed to config client TLS")
            .connect()
            .await
    ...
    let grpc_storage = rt.block_on(async {
        backend::grpc::GrpcStorageClient::new_mtls(&format!("https://{}", silo_cfg.control_plane.address)).await
    }).expect("Failed to connect to internal gRPC Storage");

    let handler = ApiHandler::new(Arc::new(grpc_storage), control_client);
    let req_metric = prometheus::register_int_counter!("req_counter", "Number of requests").expect("Failed to register metric");

    // Initialize OIDC authenticator if enabled
    let oidc_auth = if silo_cfg.auth.oidc.enabled {
        info!("OIDC authentication enabled (issuer: {})", silo_cfg.auth.oidc.issuer);
    ...
    let mut lb = pingora_proxy::http_proxy_service(&server.configuration, Gateway { handler, req_metric, oidc_auth });
    ...
    if silo_cfg.gateway.tls.enabled {
        ...
        if std::path::Path::new(cert_path).exists() {
            ...
            let mut tls_settings =
                pingora_core::listeners::tls::TlsSettings::with_callbacks(callbacks).expect("Failed to load TLS settings");
            
            tls_settings.set_certificate_chain_file(cert_path).expect("Failed to load cert chain");
            tls_settings.set_private_key_file(key_path, pingora_openssl::ssl::SslFiletype::PEM).expect("Failed to load private key");
            ...
            if let Some(ca_path) = &silo_cfg.control_plane.tls.ca_cert.clone().into() {
                ...
                if std::path::Path::new(ca_path).exists() {
                    info!("mTLS enabled: Verifying client certificates against {}", ca_path);
                    ...
                    tls_settings.set_ca_file(ca_path).expect("Failed to load client CA");
                }
            }

            tls_settings.enable_h2();
            lb.add_tls_with_settings(&silo_cfg.gateway.address, None, tls_settings);
            info!("Gateway starting: HTTPS {} (Backend: {})", silo_cfg.gateway.address, silo_cfg.storage.storage_type);
        } else {
            error!("TLS certificates not found at {}, starting without TLS", cert_path);
            lb.add_tcp(&silo_cfg.gateway.address); 
        }
    } else {
        info!("Gateway starting: HTTP {} (Backend: {})", silo_cfg.gateway.address, silo_cfg.storage.storage_type);
        lb.add_tcp(&silo_cfg.gateway.address);
    }
    ...
    info!("Metrics endpoint starting: {}", silo_cfg.gateway.metrics_address);
