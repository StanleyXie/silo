...
pub trait StorageBackend: Send + Sync {
    async fn get(&self, path: &str) -> Result<Option<Vec<u8>>, Box<dyn Error + Send + Sync>> {
        self.get_versioned(path, 0).await.map(|opt| opt.map(|(data, _)| data))
    }
...
impl StorageBackend for VaultClient {
    ...
    async fn get_versioned(&self, path: &str, version: u32) -> Result<Option<(Vec<u8>, u32)>, Box<dyn Error + Send + Sync>> {
        ...
        let resp = self.client.get(&self.url(&api_path))
        ...
        let ver_id = json.get("data").and_then(|d| d.get("metadata")).and_then(|m| m.get("version")).and_then(|v| v.as_u64()).unwrap_or(0) as u32;
    ...
    async fn put(&self, path: &str, data: &[u8], base_version: u32) -> Result<u32, Box<dyn Error + Send + Sync>> {
        ...
        let resp = self.client.put(&self.url(&api_path))
            .header("X-Vault-Token", &self.token)
            .json(&payload)
            .send()
            .await?;
        ...
        let json: Value = resp.json().await?;
        let version = json.get("data").and_then(|d| d.get("version")).and_then(|v| v.as_u64()).unwrap_or(0) as u32;
        Ok(version)
    }
    ...
    async fn delete(&self, path: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        let resp = self.client.delete(&self.url(&api_path))
            .header("X-Vault-Token", &self.token)
            .send()
            .await?;
    ...
    async fn list(&self, path: &str) -> Result<Vec<String>, Box<dyn Error + Send + Sync>> {
        ...
        let resp = self.client.request(reqwest::Method::from_bytes(b"LIST").unwrap(), &self.url(&api_path))
            .header("X-Vault-Token", &self.token)
            .send()
            .await?;
        ...
        let json: Value = resp.json().await?;
        let keys = json.get("data").and_then(|d| d.get("keys")).and_then(|k| k.as_array())
