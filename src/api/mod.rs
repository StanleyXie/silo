...
impl ApiHandler {
    ...
    pub async fn handle(&self, session: &mut Session, request_id: &str, identity: &str, client_ip: &str) -> Result<bool> {
        let path = session.req_header().uri.path().to_string();
        let method = session.req_header().method.clone();

        if !path.starts_with("/v1/state/") && !path.starts_with("/v1/lock/") && !path.starts_with("/v1/config/") {
            return Ok(false);
        }
        ...
        let local_auth = match self.client.get(&auth_path).await {
            Ok(Some(_)) => {
                info!("[{}] Autonomous Auth: Identity '{}' verified via Shared-State (KV)", request_id, identity);
                true
            },
            Ok(None) => false,
            Err(e) => {
                error!("[{}] Shared-State Lookup Error: {}. Falling back to Control Plane.", request_id, e);
                false
            }
        };
            ...
            match control.authorize(auth_req).await {
                Ok(resp) => {
                    ...
                    if !inner.authorized {
                        ...
                        session.respond_error_with_body(403, Bytes::from(format!("Forbidden: {}", inner.reason))).await?;
                        return Ok(true);
                    }
                },
                Err(e) => {
                    error!("[{}] CP Offline & Not in local KV: {}", request_id, e);
                    session.respond_error_with_body(403, Bytes::from(format!("Unauthorized: No policy found locally and CP is offline"))).await?;
                    return Ok(true);
                }
            }
        }
        ...
        if path.starts_with("/v1/state/") {
             return self.handle_state(session, &method, &path, request_id, identity).await;
        } else if path.starts_with("/v1/lock/") {
             return self.handle_lock(session, &method, &path, request_id, identity, client_ip).await;
        } else if path.starts_with("/v1/config/") {
             return self.handle_config(session, &method, &path, request_id, identity).await;
        }

        Ok(false)
    }

    async fn handle_state(&self, session: &mut Session, method: &http::Method, path: &str, request_id: &str, identity: &str) -> Result<bool> {
        ...
        match *method {
            http::Method::GET => {
                ...
                match self.client.get(&vault_path).await {
                    Ok(Some(data)) => {
                        session.respond_error_with_body(200, Bytes::from(data)).await?;
                    },
                    Ok(None) => {
                         let _ = session.respond_error_with_body(404, Bytes::from_static(b"Not Found")).await;
                    },
                    Err(e) => {
                        error!("[{}] Storage Get Error: {}", request_id, e);
                        session.respond_error_with_body(500, Bytes::from(format!("Error: {}", e))).await?;
                    }
                }
            },
            http::Method::POST => {
                ...
                if let Ok(Some(lock_session_bytes)) = self.client.get(&lock_session_path).await {
                    let locked_session_id = String::from_utf8_lossy(&lock_session_bytes).to_string();
                    
                    let device_id = session.get_header("X-Silo-Device-ID")
                        .and_then(|v| v.to_str().ok())
                        .unwrap_or("unknown-device");
                    let current_session_id = format!("{}-{}", identity.replace(".", "-"), device_id);
                    ...
                    if locked_session_id != current_session_id {
                        ...
                        session.respond_error_with_body(423, Bytes::from(format!("Locked: State is held by another session ({})", locked_session_id))).await?;
                        return Ok(true);
                    }
                }

                // --- CAS & Version Lineage ---
                let config_version = session.get_header("X-Silo-Config-Version")
                    .and_then(|v| v.to_str().ok())
                    .and_then(|s| s.parse::<u32>().ok());
                
                let base_version = session.get_header("X-Silo-Base-Version")
                    .and_then(|v| v.to_str().ok())
                    .and_then(|s| s.parse::<u32>().ok())
                    .unwrap_or(0);
                ...
                match self.client.put(&vault_path, &body, base_version).await {
                     Ok(state_version) => {
                         if let Some(cv) = config_version {
                             let lineage_path = format!("secret/lineage/{}/v{}", state_path, state_version);
                             let lineage_data = serde_json::json!({
                                 "config_version": cv,
                                 "state_version": state_version,
                                 "timestamp": chrono::Utc::now().to_rfc3339(),
                             });
                             let _ = self.client.put(&lineage_path, lineage_data.to_string().as_bytes(), 0).await;
                             info!("[{}] Lineage recorded: State v{} -> Config v{}", request_id, state_version, cv);
                         }
                         session.respond_error_with_body(200, Bytes::from_static(b"")).await?;
                     },
                     Err(e) => {
                         let err_str = e.to_string();
                         if err_str.contains("CAS") || err_str.contains("version") {
                             error!("[{}] CAS Conflict: {}", request_id, err_str);
                             session.respond_error_with_body(409, Bytes::from(format!("Conflict: {}", err_str))).await?;
                         } else {
    ...
    async fn handle_lock(&self, session: &mut Session, method: &http::Method, path: &str, request_id: &str, identity: &str, client_ip: &str) -> Result<bool> {
        ...
        match *method {
            http::Method::POST | http::Method::PUT => {
                ...
                let op = lock_info.as_ref().map(|li| li.Operation.as_str()).unwrap_or("-");
                let id = lock_info.as_ref().map(|li| li.Who.as_str()).unwrap_or("-");

                info!("[{}] LOCK: {} (caller: {}, op: {}, id: {})", request_id, lock_path, caller, op, id);
                ...
                let _ = self.client.put(&lock_meta_path, session_id.as_bytes(), 0).await;
    ...
    async fn handle_config(&self, session: &mut Session, method: &http::Method, path: &str, request_id: &str, _identity: &str) -> Result<bool> {
        let config_path = format!("secret/configs/{}", path.trim_start_matches("/v1/config/"));
        let query = session.req_header().uri.query().unwrap_or("");
        let version = query.split('&')
        ...
        match *method {
            http::Method::GET => {
                info!("[{}] GET Config: {} (v{})", request_id, config_path, version);
                match self.client.get_versioned(&config_path, version).await {
                    Ok(Some((data, ver_id))) => {
                        session.respond_error_with_body(200, Bytes::from(data)).await?;
                    },
                    Ok(None) => {
                         let _ = session.respond_error_with_body(404, Bytes::from_static(b"Config Not Found")).await;
                    },
                    Err(e) => {
